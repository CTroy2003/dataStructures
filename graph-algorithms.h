...
std::list<value_type<T>> dijkstrasAlgorithm(const WeightedGraph<T>& graph, vertex_type<T> initial_node, vertex_type<T> destination_node)
{
    ...
    while (!q.empty()){
        ...
        for (auto pair : graph.at(u)){
            ...
            bool relaxed = relax<T>(u,v,w,distances,predecessors);
            if (relaxed){
...
std::list<value_type<T>> topologicalSort(const WeightedGraph<T>& graph)
{
    ...
    for (auto [vertex, indegree] : indegrees){
        if (indegree == 0){
            q.push(vertex);
        }
    }

    int topologicalNum = 0;

    while (!q.empty()){
        value_type<T> current = q.front();
        q.pop();
        l.push_back(current);

        topological_numbers[current] = topologicalNum++;

        for (auto connected : graph.at(current)){
            auto connectedKey = connected.first;
            indegrees[connectedKey]--;
            if (indegrees[connectedKey] == 0){
                q.push(connectedKey);
            }
        }
    }
    ...
    if (l.size() != graph.size()){
        throw std::out_of_range("graph malfunctioned");
    }
    return l;
}
